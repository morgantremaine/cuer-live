...
serve(async (req) => {
  ...
  try {
    console.log('Delete test user function called');
    ...
    // Only allow deletion of test accounts
    if (!email.includes('test') && !email.includes('cuertest')) {
      return new Response(
        JSON.stringify({ error: 'This function only allows deletion of test accounts' }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400,
        }
    ...
    // Delete user's rundowns (or you could transfer them)
    ...
    console.error('Error in delete-test-user function:', error)
