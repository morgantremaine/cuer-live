...
-- Add team_id and visibility columns to rundowns table
ALTER TABLE rundowns 
ADD COLUMN team_id UUID REFERENCES teams(id) ON DELETE SET NULL,
ADD COLUMN visibility VARCHAR(20) DEFAULT 'private' CHECK (visibility IN ('private', 'team'));

-- Create indexes for better performance
CREATE INDEX idx_team_members_team_id ON team_members(team_id);
CREATE INDEX idx_team_members_user_id ON team_members(user_id);
CREATE INDEX idx_rundowns_team_id ON rundowns(team_id);
CREATE INDEX idx_rundowns_visibility ON rundowns(visibility);
